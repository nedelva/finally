...
The keys live in `.claude/settings.json` (project) or `/Users/valeriu/AICourses/finally/.claude/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/Users/valeriu/AICourses/finally/.claude/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
