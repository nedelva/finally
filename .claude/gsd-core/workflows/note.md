...
- **Global scope**: `/Users/valeriu/AICourses/finally/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/valeriu/AICourses/finally/.claude/notes/`)
...
2. Glob `/Users/valeriu/AICourses/finally/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/valeriu/AICourses/finally/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/valeriu/AICourses/finally/.claude/notes/` — works in any directory
